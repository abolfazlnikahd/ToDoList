...
class TaskService:
    ...
    def add_task(self, project_id: int, title: str, description: str = "", deadline: datetime | None = None) -> Task:
        project = self.project_repo.get_by_id(project_id)
        if not project:
            raise NotFoundError("Project not found.")
        if len(project.tasks) >= MAX_NUMBER_OF_TASK:
            raise ValidationError("Maximum number of tasks in project reached.")
        if not _words_count_ok(title, 30) or not _words_count_ok(description or "", 150):
        ...
        task = Task(project_id=project_id, title=title, description=description, status="todo", deadline=deadline)
        self.task_repo.add(task)
        return task

    def edit_task(self, project_id: int, task_id: int, title: str, description: str, status: str, deadline: datetime | None = None) -> Task:
        project = self.project_repo.get_by_id(project_id)
        if not project:
            raise NotFoundError("Project not found.")
